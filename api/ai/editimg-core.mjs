...
async function ask() {
  ...
  if (taskRes.json?.code === 6101) {
    // Token habis, reset session dan retry dengan device baru
    await fs.writeFile(SESSION_FILE, JSON.stringify({ deviceId: createDeviceId(), identityId: "", usedCount: 0 }), "utf8");
...
async function askWithRetry(retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      return await ask();
    } catch (e) {
      if (e.message === "__RETRY__" && i < retries - 1) continue;
      throw e;
    }
  }
}

askWithRetry().then(r => { process.stdout.write(JSON.stringify(r) + "\n"); }).catch(e => { process.stdout.write(JSON.stringify({ Status: false, Error: e.message }) + "\n"); process.exit(1); });
