...
// Inject fetch ke global agar WASM bisa menggunakannya
...
async function initWasm(session) {
  ...
  const res = await fetch(WASM_URL, { headers: { 'user-agent': ua } });
  if (!res.ok) throw new Error(`Gagal download WASM: ${res.status}`);
...
async function getSigns(body) {
  ...
  const result = wasmInstance.get_signs(body, tsPtr, tsLen);
...
async function apiPost(session, endpoint, body = {}) {
  ...
  return { code: res.status, json: JSON.parse(text) };
}
...
async function editImage(imageUrl, prompt) {
  ...
  if (!s3Res.ok) throw new Error(`Upload S3 gagal: ${s3Res.status}`);
