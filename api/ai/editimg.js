...
async function getSigns(body) {
  ...
  // body harus di-pass sebagai JS object reference (externref) ke WASM
  const bodyRef = addExternRef(body);

  const timestampPtr = passStringToWasm(timestamp, wasmInstance.__wbindgen_malloc, wasmInstance.__wbindgen_realloc);
  const timestampLen = wasmLastLen;

  const result = wasmInstance.get_signs(bodyRef, timestampPtr, timestampLen);
  const ptr = result[0];
  const len = result[1];

  const text = readWasmString(ptr, len);
  wasmInstance.__wbindgen_free(ptr, len, 1);
