...
const meta = {
    ...
    params: [
        ...
        { name: 'nama1',           placeholder: 'Nama item ke-1'       },
        { name: 'qty1',            placeholder: '1'                    },
        { name: 'satuan1',         placeholder: 'pcs (opsional)'       },
        { name: 'harga1',          placeholder: '15000'                },
        { name: 'nama2',           placeholder: 'Nama item ke-2 (dst)' },
        { name: 'bayar',           placeholder: '100000'               },
...
function parseItemsFromBody(items) {
...
async function generatePDF(data = {}) {
    ...
    const baseName        = `struk-${safeFileName(toko.nama || 'toko')}-${now.fileTanggal}-${now.fileJam}-${Date.now().toString().slice(-6)}`;
    ...
    return { pdfPath, pngPath, baseName, tanggal: now.tanggal, jam: now.jam, nomorStruk: finalNomorStruk, totalQty, subTotal, total, bayar: Number(bayar || 0), kembali };
}

// ======================
// PDF → PNG via Ghostscript
// ======================
function pdfToPng(pdfPath, pngPath, dpi = 200) {
    execSync(
        `ghostscript -dNOPAUSE -dBATCH -sDEVICE=png16m -r${dpi} -dFITPage -sOutputFile="${pngPath}" "${pdfPath}"`,
        { timeout: 30000 }
    );
}
...
async function handler(req, res) {
    ...
    const {
        toko, alamatToko, kotaToko, telpToko,
        kasir, pelanggan, alamatPelanggan,
        nomorStruk, bayar, metodeBayar
    } = body;

    // parse items
    let parsedItems = [];
    try {
        if (isPost && body.items) {
            parsedItems = parseItemsFromBody(body.items);
        } else {
            parsedItems = parseItemsFromQuery(body);
        }
    } catch (e) {
        return res.status(400).json({ status: false, message: `Format items tidak valid: ${e.message}` });
    }
    ...
    if (!parsedItems || parsedItems.length === 0) {
        return res.status(400).json({
            ...
            contohGet: '/api/maker/struk?toko=Karis Jaya&kasir=Admin&pelanggan=Budi&bayar=20000&metodeBayar=Cash&nama1=Ayam Bakar&qty1=1&harga1=15000&nama2=Es Teh&qty2=1&harga2=5000'
        });
    }
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `inline; filename="${path.basename(pngPath)}"`);
